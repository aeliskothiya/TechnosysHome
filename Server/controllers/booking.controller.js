...
export async function generateCompletionOTP(req, res) {
  try {
    ...
    // Send email to customer (non-blocking with retry)
    try {
      const customer = await Customer.findById(booking.CustomerID).lean();
      const email = customer?.Email;
      if (email) {
        ...
        if (EMAIL_DEV_MODE || !SMTP_READY) {
          console.log('[generateCompletionOTP] Skip email (dev mode or SMTP not configured)', { otp });
        } else {
          // Fire-and-forget so the API responds immediately; retries happen inside helper
          sendEmailWithRetry({
            from: `${SENDER_NAME} <${SENDER_EMAIL}>`,
          ...
          if (process.env.NODE_ENV === 'development') {
            console.log(`📧 Queued completion OTP to ${email}. OTP: ${otp}`);
          }
        }
      }
    } catch (mailErr) {
      console.warn("Failed to queue completion OTP email", mailErr);
    }
