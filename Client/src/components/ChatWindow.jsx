                  ...
                  {Array.isArray(m.Attachments) && m.Attachments.length > 0 && (
                    <div className={`grid gap-2 ${m.MessageText ? 'mt-2' : ''}`} style={{
                      ...
                      {m.Attachments.map((a, idx) => (
                        <img 
                          key={idx} 
                          src={`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:6001'}/${a.path}`} 
                          alt={a.originalName}
                          ...
                          onClick={() => window.open(`${import.meta.env.VITE_API_BASE_URL || 'http://localhost:6001'}/${a.path}`, '_blank')}
                        />
                      ))}
                    </div>
                  )}
